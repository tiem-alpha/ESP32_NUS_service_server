...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "Tiem NUS";
...
static uint16_t tx_cccd_value = 0x0000;
static bool nus_connected = false;

static esp_gatt_char_prop_t nus_rx_property = 0;
static esp_gatt_char_prop_t nus_tx_property = 0;

static uint8_t nus_rx_init_val[GATTS_DEMO_CHAR_VAL_LEN_MAX] = {0};
static uint8_t nus_tx_init_val[GATTS_DEMO_CHAR_VAL_LEN_MAX] = {0};

static esp_attr_value_t nus_rx_char_val = {
    .attr_max_len = GATTS_DEMO_CHAR_VAL_LEN_MAX,
    .attr_len = 0,
    .attr_value = nus_rx_init_val,
};

static esp_attr_value_t nus_tx_char_val = {
    .attr_max_len = GATTS_DEMO_CHAR_VAL_LEN_MAX,
    .attr_len = 0,
    .attr_value = nus_tx_init_val,
};
...
static uint8_t raw_adv_data[] = {
    ...
    /* Complete 128-bit NUS Service UUID */
...
static uint8_t raw_scan_rsp_data[] = {
    /* Complete Local Name */
    0x09, ESP_BLE_AD_TYPE_NAME_CMPL, 'T', 'i', 'e', 'm', ' ', 'N', 'U', 'S',
    /* TX Power Level */
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid128, nus_service_uuid128, sizeof(nus_service_uuid128));
        ...
        adv_config_done |= scan_rsp_config_flag;
#endif
        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_A_APP_ID].service_id, GATTS_NUM_HANDLE_NUS);
        break;
    case ESP_GATTS_READ_EVT: {
        ESP_LOGI(GATTS_TAG,
                    "GATT read request: conn_id=%d, trans_id=%" PRIu32 ", handle=%d, is_long=%d, offset=%d, need_rsp=%d",
        ...
        if (param->read.handle == gl_profile_tab[PROFILE_A_APP_ID].descr_handle) {
            memcpy(rsp.attr_value.value, &tx_cccd_value, sizeof(tx_cccd_value));
            rsp.attr_value.len = sizeof(tx_cccd_value);
    ...
    case ESP_GATTS_WRITE_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT write request, conn_id %d, trans_id %" PRIu32 ", handle %d, len %d",
        ...
        if (!param->write.is_prep){
            ...
            if (param->write.handle == gl_profile_tab[PROFILE_A_APP_ID].char_handle) {
                ...
                if (write_len > GATTS_DEMO_CHAR_VAL_LEN_MAX) {
                    write_len = GATTS_DEMO_CHAR_VAL_LEN_MAX;
                }
                memcpy(nus_rx_init_val, param->write.value, write_len);
                nus_rx_char_val.attr_len = write_len;
                memcpy(nus_tx_init_val, param->write.value, write_len);
                nus_tx_char_val.attr_len = write_len;
                ESP_LOGI(GATTS_TAG, "NUS RX received %d bytes", write_len);

                if (nus_connected && gl_profile_tab[PROFILE_A_APP_ID].tx_char_handle != 0 && (tx_cccd_value & 0x0001)) {
                    uint16_t notify_len = write_len;
                    uint16_t max_notify_len = (local_mtu > 3) ? (local_mtu - 3) : 0;
                    if (notify_len > max_notify_len) {
                        notify_len = max_notify_len;
                    }
                    if (notify_len > 0) {
            ...
            } else if (param->write.handle == gl_profile_tab[PROFILE_A_APP_ID].descr_handle && param->write.len == 2) {
                tx_cccd_value = param->write.value[1] << 8 | param->write.value[0];
                ESP_LOGI(GATTS_TAG, "TX CCCD %s", (tx_cccd_value == 0x0001 ? "enabled" : "disabled"));
            }
        }
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid128, nus_rx_char_uuid128, sizeof(nus_rx_char_uuid128));
        nus_rx_property = ESP_GATT_CHAR_PROP_BIT_WRITE | ESP_GATT_CHAR_PROP_BIT_WRITE_NR;
        esp_err_t add_char_ret = esp_ble_gatts_add_char(gl_profile_tab[PROFILE_A_APP_ID].service_handle,
                                                        &gl_profile_tab[PROFILE_A_APP_ID].char_uuid,
                                                        ESP_GATT_PERM_WRITE,
                                                        nus_rx_property,
                                                        &nus_rx_char_val, NULL);
        if (add_char_ret) {
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
        ...
        if (gl_profile_tab[PROFILE_A_APP_ID].char_handle == 0) {
            ...
            gl_profile_tab[PROFILE_A_APP_ID].char_uuid.len = ESP_UUID_LEN_128;
            memcpy(gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid128, nus_tx_char_uuid128, sizeof(nus_tx_char_uuid128));
            nus_tx_property = ESP_GATT_CHAR_PROP_BIT_NOTIFY | ESP_GATT_CHAR_PROP_BIT_READ;
            esp_err_t add_tx_ret = esp_ble_gatts_add_char(gl_profile_tab[PROFILE_A_APP_ID].service_handle,
                                                          &gl_profile_tab[PROFILE_A_APP_ID].char_uuid,
                                                          ESP_GATT_PERM_READ,
                                                          nus_tx_property,
                                                          &nus_tx_char_val, NULL);
            if (add_tx_ret) {
                ESP_LOGE(GATTS_TAG, "add TX char failed, error code =%x", add_tx_ret);
            }
        } else {
            ...
            esp_err_t add_descr_ret = esp_ble_gatts_add_char_descr(gl_profile_tab[PROFILE_A_APP_ID].service_handle,
                                                                  ...
                                                                  NULL, NULL);
            if (add_descr_ret) {
                ESP_LOGE(GATTS_TAG, "add TX descriptor failed, error code =%x", add_descr_ret);
            }
        }
        break;
    }
...
/* Profile B removed: using single Nordic UART Service (NUS) profile only */
